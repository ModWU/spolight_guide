...
| `SpotlightGuideBarrierStyle`, blur, color, hole pass-through, barrier hit testing | `src/api/models.dart`, `src/layout/overlay_layout.dart`, `src/painting/barrier_painter.dart` | `doc/reference.md`, `doc/troubleshooting.md`, `README.md`, `CHANGELOG.md` | `spotlight_guide_hint_test.dart` |
...
| Pointer image, `SpotlightGuideBubbleHint`, `pointerAnchorPosition`, bubble body offset | `src/hints/bubble_hint.dart` | `doc/reference.md`, `doc/examples.md` if usage changes | `spotlight_guide_pointer_test.dart` |
...
| Low-level decoration or painter behavior | `src/hints/bubble_decoration.dart`, `src/painting/` | `doc/reference.md`, `doc/troubleshooting.md` if symptoms change | Layout/pointer tests that inspect geometry |
